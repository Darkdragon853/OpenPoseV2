			...
			if (numberBodyParts > 0)
			{
				...
				for (auto part = 0; part < numberBodyParts; ++part)
				{
					...
					for (auto i = 0u; i < keypointsVector.size(); i++)
					{
						...
						// the current values are in keypoints.at(0) and (1)
						...
						if (!keypoints.empty() && isValidKeypoint(&keypoints[baseIndex], imageSizes[i]))
						{
							...
							opLog("Keypoints was empty or not valid keyPoints.");
						}
					}
				...
				// 3D reconstruction
				opLog("3D-Reconstruction Starts");
				...
				if (!xyPoints.empty())
				{
					...
					for (auto i = 0u; i < xyPoints.size(); i++)
					{
						...
						opLog("Triangulation done with X Y Z:");
						opLog(reconstructedPoint.at<double>(0));
						opLog(reconstructedPoint.at<double>(1));
						opLog(reconstructedPoint.at<double>(2));
					...
					opLog("Start Erasing Points which aren't in the Reprojection-Error-Trashhold");
					for (auto index = 0u; index < indexesUsed.size(); ++index)
					{
						if (std::isfinite(xyzPoints[index].x) && std::isfinite(xyzPoints[index].y)
							&& std::isfinite(xyzPoints[index].z)	) // Debug: Ignore ReprojectionError to get all results - get it back working of the Calibration is better 
							// Remove outliers
					...
					opLog("xyPoints was emtpy.");
				}
				return false;
			}
			// Keypoints in < 2 images
			else
			opLog("Failed hard bc OpenPose thinks there are no people in the image");
